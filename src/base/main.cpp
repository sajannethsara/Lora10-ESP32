...
void setup()
{
    ...
    if (loraQueue == NULL)
    {
        Serial.println("ERROR: Failed to create LoRa queue");
        while (1) delay(1000);
    }
...
void _LoRaListenTask(void *pvParameters)
{
    ...
    while (1)
    {
        try
        {
            ...
            {
                lastLoRaActivity = millis();
                Serial.printf("[LoRa] Received packet (size: %d)\n", packetSize);
                ...
                if (valid)
                {
                    ...
                    setMostRecentMsg(msg, uid);
                    std::string jsonPayload = baseDevice.getJsonPayload();
                    ...
                    // Non-blocking scroll text (simplified)
        ...
        {
            Serial.println("[LoRa] Exception in LoRa Listen Task");
        }
        
        vTaskDelay(pdMS_TO_TICKS(20)); // Reduced delay for better responsiveness
    }
}
        ...
        try
        {
            ...
            if (xQueueReceive(loraQueue, &receivedPayload, portMAX_DELAY) == pdTRUE)
            {
                ...
                if (receivedPayload.length > 0 && receivedPayload.length <= sizeof(receivedPayload.data))
                {
                    ...
                    // Clear the vector to free memory immediately
                    payloadVector.clear();
